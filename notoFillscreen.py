...
class Noto:
    def __init__(self, fontSize=12, text="🖤🖤❤⬛︎", color="white", screen_x=1920, screen_y=1080, background="black"):
        ...
        font.init()
        self.fontSet = font.Font("NotoEmoji-Regular.ttf", size=self.fontSize)
    ...
    def render_simple(self):
        ...
        return font.Font.render(self.fontSet, self.text,True,self.color,None)
    ...
    def text_jumbler(self):
        ...
        for x in range(0,x_length):
            textReturn = textReturn + str(textSplit[random.randint(0,textLen)])
        return textReturn
    ...
    def render_advanced(self):
        ...
        for y in range(0,len(self.emojiArray)):
            fontHold = font.Font.render(self.fontSet,self.emojiArray[y],True,self.color,self.background)
